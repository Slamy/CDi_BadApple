...
short CountUsedPCLs() {
    ...
    if (current_seqnum > 50 &&
        current_seqnum < (EXPECTED_NUMBER_OF_FRAMES - 20)) {
...
void initPcls() {
    ...
    for (i = 0; i < VIDEO_PCL_COUNT; i++) {
        initMpegPcl(&(mvPcl[i]), MV_SIG_PCL, &(mvPcl[(i + 1) % VIDEO_PCL_COUNT]),
                    address, 1);
        address += SECTOR_SIZE;
    }
}
    ...
    for (;;) {
        ...
        if ((magic & 0xfffe) == 0x4242) {
            if (seqnum > current_seqnum) {
                ...
                /* We aren't allowed to continue here, since we have lost our working
                 * buffer */
                return 0;
            }
